...
int main(int argc, char* argv[]) {
    ...
    Mix_Music* menuMusic = Mix_LoadMUS("assets/music/bg.wav");
    SDL_Texture* menuBg = loadTexture(renderer, "assets/images/background.png");
    TTF_Font* font2 = TTF_OpenFont("assets/fonts/WinkyRough-Light.ttf", 42);
    int menuChoice = showMenu(renderer, font2, menuBg, menuMusic);
    if (menuChoice == 1) {
        // bắt đầu chơi
    }
    ...
    int highScore = loadHighScore("F:\\game\\gamerun\\include\\highscore.txt");
    while (running) {
        ...
        if (SDL_HasIntersection(&gaRect, &cactusRect)) {
            ...
            if (score > highScore) {
                highScore = score;
                saveHighScore("F:\\game\\gamerun\\include\\highscore.txt", highScore);
            }
        ...
        SDL_Texture* hsTexture = renderText(renderer, font, hsText, textColor);
        int hsW, hsH;
        SDL_QueryTexture(hsTexture, nullptr, nullptr, &hsW, &hsH);
        SDL_Rect hsRect = { SCREEN_WIDTH - hsW - 10, 10, hsW, hsH };
